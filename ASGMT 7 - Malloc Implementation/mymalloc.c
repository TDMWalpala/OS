...
void init(){
    freelist->size = 25000 - sizeof(struct info);//decreasing memory for the first meta data block
    freelist->free = 1;
...
void merge(){
    ...
    while(current->next){ //looping through memory from the beginning
...
void split(struct info* slot,size_t blocksize){
    ...
    new->size = slot->size - blocksize - sizeof(struct info);//separating the extra memory block as "new"
    new->free = 1;
...
void* MyMalloc(size_t blocksize){
    ...
    if(current->size == blocksize){
        ...
        current = current + 1;
        result = (void*)current;
        printf("Block allocation successful\n");
        return result;
    }
    /*if there is a suitable block but the size is larger than required.Allocated memory block using split*/
    ...
    /*if code gets here that means there isn't a suitable block*/
        ...
        result = NULL;
        printf("Block allocation unsuccessful!\n");
        return result;
    }
}
